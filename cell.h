...
class cell {
public:
    cell(bool l = false, bool r = false, bool u = false, bool d = false, int c = 0, char n = ' ', bool v = false, int x = 0, int y = 0, cell* prnt = nullptr, char s = ' ') : left(l), right(r), above(u), below(d), cost(c), name(n), visited(v), coordinate_x(x), coordinate_y(y), parent(prnt),symbol(s) {};
    cell(const cell& c) {
        left = c.left;
        right = c.right;
        above = c.above;
